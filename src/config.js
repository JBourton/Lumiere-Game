...
export const VISTOR_MOVE_SPEED = 2000; //how fast visitor can go around durham
export const VISITOR_REDRAW_INTERVAL = 100; // (not currently used but useful for improving framerate @ high visitor cnt)
export const VISITOR_CAP = 70; // the most visitors possible for a game (so as not to crash it)
...
export function get_spawn_interval_from_frustration() {
    ...
    if (tier_of_frust === 1) return 10000;   // i.e. every 10s
    if (tier_of_frust === 2) return 20000;   // and every 20s
    if (tier_of_frust === 3) return 30000;   // then a whole 40s
